...
$$
    \textbf{w}.\textbf{x} + b = 0
$$
where $\textbf{w}$ is the weights, $\textbf{x}$ is the data and $b$ is the bias 

The rule of the linear classifier is to assign a class $c$ to data $x$: 
$$
    c = \begin{cases}
        + 1 & \text{if } $\textbf{w}.\textbf{x}$ + b \geq 0 \\
        - 1 & \text{otherwise}
    \end{cases}
$$

To train the classifier, the parameters $\textbf{w}$ and $b$ determines the
decision boundary and needs to be estimated.

Intuitively, the best hyperplane (line) is one that is far from both classes of
points i.e. \textbf{maximum margin hyperplane}. This line can be determined from
only the innermost points i.e. \textbf{support vectors}.
\begin{figure}[h]
    \centering
    \includegraphics[width=8cm]{Hyperplane.JPG}
\end{figure}

Ideally, the support vectors should fulfil the equations $\textbf{w}.\textbf{x}
+ b = 1$ or $-1$, so that all the other pointsare easily classified

\pagebreak

\subsubsection{Maximum margin}
